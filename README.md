...
The browser ActiveTrack camera and recorded-video route use the same basketball-trained YOLO detector, ByteTrack settings, confidence gate, and pure scoring state machine. The live browser camera automatically finds the hoop and keeps its tracked identity while following the ball and primary player. The video route accepts a device-library recording up to five minutes long, automatically selects a clear hoop frame, and analyzes decoded frames at 30 FPS; manual rim adjustment remains available only as a correction. Uploaded tracks are cleaned in rim-relative coordinates before the shared state machine fits or interpolates the descending rim-plane crossing. A make requires a complete above-rim approach and centered exit below the net. A miss is delayed until the completed below-net trajectory proves that the ball exited outside the opening, preventing a single jittery rim frame from creating a false miss. Low-confidence decisions remain uncounted for review.
